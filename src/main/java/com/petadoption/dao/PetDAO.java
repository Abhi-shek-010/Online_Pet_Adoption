...
public class PetDAO {
    ...
    // SQL Query Constants
    private static final String INSERT_PET = "INSERT INTO pets (shelter_id, pet_name, species, breed, age_years, age_months, gender, weight_kg, color, health_status, vaccination_status, neutered_spayed, microchip_number, description, special_needs, adoption_status, adoption_fee, image_url, intake_date) "
            +
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    private static final String SELECT_PET_BY_ID = "SELECT p.*, u.full_name as shelter_name FROM pets p JOIN users u ON p.shelter_id = u.user_id WHERE p.pet_id = ?";

    private static final String SELECT_ALL_AVAILABLE_PETS = "SELECT p.*, u.full_name as shelter_name FROM pets p JOIN users u ON p.shelter_id = u.user_id WHERE p.adoption_status = 'AVAILABLE' ORDER BY p.created_at DESC";

    private static final String SELECT_PETS_BY_SHELTER = "SELECT p.*, u.full_name as shelter_name FROM pets p JOIN users u ON p.shelter_id = u.user_id WHERE p.shelter_id = ? ORDER BY p.created_at DESC";

    private static final String SELECT_PETS_BY_STATUS = "SELECT p.*, u.full_name as shelter_name FROM pets p JOIN users u ON p.shelter_id = u.user_id WHERE p.adoption_status = ? ORDER BY p.created_at DESC";

    private static final String SELECT_PETS_BY_SPECIES = "SELECT p.*, u.full_name as shelter_name FROM pets p JOIN users u ON p.shelter_id = u.user_id WHERE p.species = ? AND p.adoption_status = 'AVAILABLE' ORDER BY p.pet_name ASC";

    private static final String SELECT_PETS_BY_BREED = "SELECT p.*, u.full_name as shelter_name FROM pets p JOIN users u ON p.shelter_id = u.user_id WHERE p.breed = ? AND p.adoption_status = 'AVAILABLE'";

    private static final String SELECT_ALL_PETS = "SELECT p.*, u.full_name as shelter_name FROM pets p JOIN users u ON p.shelter_id = u.user_id ORDER BY p.created_at DESC";

    private static final String UPDATE_PET = "UPDATE pets SET pet_name = ?, breed = ?, age_years = ?, age_months = ?, weight_kg = ?, color = ?, health_status = ?, vaccination_status = ?, neutered_spayed = ?, microchip_number = ?, description = ?, special_needs = ?, adoption_status = ?, adoption_fee = ?, image_url = ?, updated_at = NOW() "
            +
            "WHERE pet_id = ?";

    private static final String UPDATE_PET_ADOPTION_STATUS = "UPDATE pets SET adoption_status = ?, adoption_date = ?, updated_at = NOW() WHERE pet_id = ?";

    private static final String DELETE_PET = "DELETE FROM pets WHERE pet_id = ?";

    private static final String COUNT_AVAILABLE_PETS_BY_SHELTER = "SELECT COUNT(*) FROM pets WHERE shelter_id = ? AND adoption_status = 'AVAILABLE'";

    private static final String SEARCH_PETS = "SELECT p.*, u.full_name as shelter_name FROM pets p JOIN users u ON p.shelter_id = u.user_id WHERE p.adoption_status = 'AVAILABLE' AND (p.pet_name LIKE ? OR p.species LIKE ? OR p.breed LIKE ?) ORDER BY p.pet_name ASC";
     ...
     * @throws SQLException if database operation fails
     */
    public boolean updateAdoptionStatus(int petId, Pet.AdoptionStatus newStatus, LocalDate adoptionDate)
            throws SQLException {
        ...
        try {
            connection = DBConnection.getInstance().getConnection();
            return updateAdoptionStatus(connection, petId, newStatus, adoptionDate);
        } finally {
            DBConnection.closeConnection(connection);
        }
    }
    ...
    /**
     * Updates pet's adoption status (transaction-aware - uses provided connection)
     * This overload is used within transactions to ensure atomic operations.
     * 
     ...
     * @throws SQLException if database operation fails
     */
    public boolean updateAdoptionStatus(Connection connection, int petId, Pet.AdoptionStatus newStatus,
            LocalDate adoptionDate) throws SQLException {
        if (petId <= 0 || newStatus == null) {
            throw new IllegalArgumentException("Valid pet ID and status required");
        }
        ...
        try {
            ...
            DBConnection.closePreparedStatement(preparedStatement);
            // Do NOT close connection here - it's managed by the transaction
        }
    ...
    private Pet mapResultSetToPet(ResultSet resultSet) throws SQLException {
        ...
        // Populate shelter name if available in result set
        try {
            String shelterName = resultSet.getString("shelter_name");
            if (shelterName != null) {
                pet.setShelterName(shelterName);
            }
        } catch (SQLException e) {
